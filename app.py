...
risk_score = min(max((bag + 20) * 2, 0), 100)   # scaled 0–100
...
st.write(f"### Risk Score: {int(risk_score)}%")
...
ax.set_title("Predicted Disease Likelihood")
st.pyplot(fig)

# ---- CONFIDENCE SCORE ----
confidence = round(max(probs.values()) * 100, 2)
...
st.subheader("🧾 Final AI Interpretation")

st.write(f"""
Most Likely Condition: **{top_disease}**
